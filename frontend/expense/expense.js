...
form.addEventListener('submit', async (event) =>{
    ...
    const response = await axios.post("http://54.91.33.84:3000/expense/add-expense",expense_details,{headers:{"Authorization":token}});
...
window.addEventListener('DOMContentLoaded', async ()=>{
    ...
    const token = localStorage.getItem('token');
    const response = await axios.get(`http://54.91.33.84:3000/expense/get-expenses?page=${page}&rows=${rows}`,{headers:{"Authorization":token}})
...
function showExpenseOnScreen(expense_details){
    ...
    deleteButton.onclick = async () =>{
       try{
        const token = localStorage.getItem('token');
         await axios.delete(`http://54.91.33.84:3000/expense/delete-expense/${expense_details.id}`,{headers:{"Authorization":token}})
...
document.getElementById('rzp-button').onclick = async (e) =>{
    try{
    const token = localStorage.getItem('token');
    const response = await axios.get('http://54.91.33.84:3000/purchase/premiumship', {headers:{"Authorization":token}});
    var options = {
        "key":response.data.key_id,
        "order_id":response.data.order.id,
        "handler": async (response) =>{
            const res = await axios.post("http://54.91.33.84:3000/purchase/update-transaction",{
