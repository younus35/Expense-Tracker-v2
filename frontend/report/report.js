...
async function getDailyReport(e) {
    try {
      ...
      const res = await axios.post(
        "http://44.220.155.153:3000/reports/dailyReports",
        {
          date: formattedDate,
        },
  ...
  async function getMonthlyReport(e) {
    try {
      ...
      let totalAmount = 0;
      const res = await axios.post(
        "http://44.220.155.153:3000/reports/monthlyReports",
        {
...
downloadBtn.addEventListener("click", async () =>{
  try{
  const token = localStorage.getItem("token");
  const response = await axios.get("http://44.220.155.153:3000/reports/download", {headers:{"Authorization":token}})
